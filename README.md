...
- 글로벌 로딩 인디케이터
- 다크/라이트/시스템 테마 지원
- 다국어 지원 (한국어/영어, 기기 언어 자동 감지)
- OTA 업데이트 (EAS Update)
- 공통 컴포넌트
  - Button, Text, Input, Divider, Screen
  - Modal (Alert/Dialog), Toast, Badge, Avatar, EmptyState, Card

## 폴더 구조
```
src/
├── app/
│   ├── _layout.tsx            # 루트 레이아웃 (인증 분기, SDK 초기화, QueryClientProvider, Toast)
│   ├── (auth)/                # 비로그인 화면
│   │   ├── _layout.tsx
│   │   └── login.tsx          # 소셜 로그인 화면
│   └── (app)/                 # 로그인 후 화면
...
│       ├── Modal.tsx          # Alert/Dialog (버튼 1개 또는 2개)
│       ├── Toast.tsx          # 상단 알림 (success/error/warning/info)
│       ├── Badge.tsx          # 텍스트/숫자 뱃지
│       ├── Avatar.tsx         # 이니셜/이미지 아바타 (다크모드 지원)
...
│   ├── languageStore.ts       # 언어 설정 (ko/en, AsyncStorage 저장)
│   └── toastStore.ts          # Toast 상태 관리
├── api/
...
│   ├── useTranslation.ts      # 다국어 훅 (t 함수 + 언어 변경 감지)
...
│   └── en.ts                  # 영어
├── constants/
│   └── theme.ts               # Colors, BrandColors, BadgeColors, AvatarColors, FontSize, Spacing, Radius 등
└── types/
